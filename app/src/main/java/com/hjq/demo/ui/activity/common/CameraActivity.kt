...
class CameraActivity : AppActivity() {
        ...
        fun start(activity: BaseActivity, video: Boolean, listener: OnCameraListener?) {
            XXPermissions.with(activity)
                ...
                .request { _, deniedList ->
                    ...
                    activity.startActivityForResult(CameraActivity::class.java, {
                        putExtra(INTENT_KEY_IN_FILE, file)
                        putExtra(INTENT_KEY_IN_VIDEO, video)
    ...
    override fun initData() {
        ...
        startActivityForResult(intent, {
            val imageUri: Uri? = if (isAndroid7()) {
                // 通过 FileProvider 创建一个 Content 类型的 Uri 文件
                FileProvider.getUriForFile(this@CameraActivity, AppConfig.getPackageName() + ".provider", file)
            } else {
                Uri.fromFile(file)
            }
