...
class ImageSelectActivity : AppActivity(), StatusAction, Runnable,
        ...
        fun start(activity: BaseActivity, maxSelect: Int, listener: OnImageSelectListener?) {
            ...
            XXPermissions.with(activity)
                ...
                .request { _, deniedList ->
                    ...
                    activity.startActivityForResult(ImageSelectActivity::class.java, {
    ...
    override fun initView() {
        ...
        recyclerView?.let {
            ...
            // 添加分割线
            it.addItemDecoration(GridSpaceDecoration(dp2px(3).toInt()))
            // 设置滚动监听
    ...
    override fun onRightClick(titleBar: TitleBar) {
        ...
        for (key: String in keys) {
            ...
            count += list.size
            data.add(AlbumInfo(list[0], key, String.format(getString(R.string.image_select_total), list.size),
                adapter.getData() !== allImage && adapter.getData() == list))
        }
    ...
    override fun run() {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            do {
                ...
                val width = cursor.getString(widthIndex)
                val height = cursor.getString(heightIndex)
                // 有些破损的图片宽高是获取不到的，这里需要直接忽略
                // 我自己测试的话，就是获取到等于 null，别人反馈的是获取到 -1
                if (width == null || height == null || "-1" == width || "-1" == height) {
                    continue
                }
        ...
        post {
            ...
            // 执行列表动画
            recyclerView?.layoutAnimation = AnimationUtils.loadLayoutAnimation(
                this@ImageSelectActivity, R.anim.layout_fall_down
            )
            recyclerView?.scheduleLayoutAnimation()
