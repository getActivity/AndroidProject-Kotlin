...
class FullScreenModeController {
    ...
    fun enterFullScreenMode(activity: Activity?, customView: View?, callback: CustomViewCallback?) {
        if (isFullScreenMode()) {
        ...
        if (activity != null) {
            ...
            val targetActivityOrientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
            ...
            if (currentActivityOrientation != targetActivityOrientation) {
                try {
                    ...
                    activity.requestedOrientation = targetActivityOrientation
