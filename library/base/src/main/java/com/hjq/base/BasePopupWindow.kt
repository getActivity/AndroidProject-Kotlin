...
open class BasePopupWindow constructor(private val context: Context) : PopupWindow(context),
    ...
    private fun setActivityAlpha(alpha: Float) {
        val activity: Activity = getContext().getActivity() ?: return
    ...
    open class Builder<B : Builder<B>> constructor(
        ...
        open fun removeOnShowListener(listener: OnShowListener?): B {
            ...
            showListeners.remove(listener)
            if (isCreated()) {
                popupWindow?.removeOnShowListener(listener)
            }
            return this as B
        }
        ...
        open fun removeOnDismissListener(listener: OnDismissListener?): B {
            ...
            dismissListeners.remove(listener)
            if (isCreated()) {
                popupWindow?.removeOnDismissListener(listener)
            }
            return this as B
        }
